...
import { ChevronRight, Sparkles, ShoppingBag, ArrowRight, Filter } from 'lucide-react';
...
const WoodenUtensils: React.FC = () => {
  ...
  const fetchWoodenUtensilsCategory = async () => {
    try {
      ...
      if (woodenUtensilsCat) {
        ...
        // Find subcategories (children of ID 63)
        const children = allCategories.filter((cat: Category) => cat.parent_id === 63);
  ...
  const fetchProducts = async (page: number) => {
    try {
      ...
      const params: any = {
        page,
        per_page: 20,
        category_id: selectedSubCategory || 63, // Use subcategory if selected, otherwise main category
      };
  ...
  if (isLoading && !category) {
    return (
      <div className="min-h-screen bg-gradient-to-b from-amber-50 to-white">
        {/* Header Skeleton */}
        <div className="relative h-64 bg-gradient-to-r from-amber-800 to-amber-600 overflow-hidden">
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-amber-50 to-white">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="flex flex-col lg:flex-row gap-8">
          {/* Left Sidebar - Category Image and Subcategories */}
          <div className="lg:w-1/4">
              ...
              {category && (
                <div className="bg-gradient-to-br from-amber-50 to-amber-100 rounded-2xl shadow-lg overflow-hidden border border-amber-200">
                  <div className="relative h-48">
                    <Image
                      src={category.image_url || '/images/wooden-utensils.jpg'}
                      alt={category.name}
                      fill
                  ...
                  <div className="p-6">
                    <h3 className="text-xl font-bold text-amber-900 mb-2">Why Wooden Utensils?</h3>
                    <ul className="space-y-2 text-amber-800">
                      <li className="flex items-start">
                        <span className="text-amber-600 mr-2">•</span>
                        <span className="text-sm">Gentle on cookware - won't scratch non-stick surfaces</span>
                      </li>
                      <li className="flex items-start">
                        <span className="text-amber-600 mr-2">•</span>
                        <span className="text-sm">Naturally anti-bacterial properties</span>
                      </li>
                      <li className="flex items-start">
                        <span className="text-amber-600 mr-2">•</span>
                        <span className="text-sm">Sustainable and eco-friendly</span>
                      </li>
                      <li className="flex items-start">
                        <span className="text-amber-600 mr-2">•</span>
                        <span className="text-sm">Heat resistant - won't melt</span>
                      </li>
                    </ul>
                  </div>
                </div>
            ...
            {isLoading ? (
              ...
              <>
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                  {products.map((product) => (
                    <div
                      key={product.id}
                      className="bg-white rounded-lg shadow-sm hover:shadow-md border border-gray-200 hover:border-amber-300 overflow-hidden transition-all duration-300 transform hover:-translate-y-1"
                    >
                      <ProductCard
                        product={product}
              ...
              <div className="text-center py-12">
                <div className="bg-amber-50 rounded-2xl p-12">
                  <ShoppingBag size={64} className="mx-auto text-amber-300 mb-4" />
