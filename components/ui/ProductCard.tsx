...
const ProductCard: React.FC<ProductCardProps> = ({ 
  ...
  return (
    <div 
      className="group relative bg-white rounded-2xl md:rounded-3xl transition-all duration-700 ease-out overflow-hidden h-full flex flex-col"
      style={{
        boxShadow: '0 4px 12px -8px rgba(0,0,0,0.05), 0 0 0 1px rgba(0,0,0,0.02)',
    ...
    >
      ...
      <div 
        className="absolute inset-0 bg-gradient-to-br from-orange-500/5 via-transparent to-amber-500/5 opacity-0 group-hover:opacity-100 transition-opacity duration-700 pointer-events-none rounded-2xl md:rounded-3xl"
        style={{ zIndex: 1 }}
      />
      
      {/* Image Section */}
      <div className="relative w-full aspect-square p-3 sm:p-4 bg-gradient-to-br from-gray-50 via-white to-gray-50">
        
        {/* Badges - Top left */}
        <div className="absolute top-2 left-2 sm:top-3 sm:left-3 z-30 flex flex-col gap-1">
          {badges.map((badge, index) => {
            ...
            return (
              <span 
                ...
                className={`
                  inline-flex items-center gap-1 sm:gap-1.5 text-[8px] sm:text-[10px] font-bold px-1.5 sm:px-2.5 py-1 sm:py-1.5 
                  rounded-full bg-gradient-to-r ${badge.gradient} text-white 
                  shadow-lg ${badge.shadow} backdrop-blur-sm
                  transform transition-all duration-300
                `}
              >
                {Icon && <Icon size={8} className="sm:w-[10px] sm:h-[10px]" />}
                {badge.content}
              </span>
            );
          })}
        ...
        {(product as any).is_bestseller && (
          <div className="absolute top-2 right-2 sm:top-3 sm:right-3 z-30">
            <span className="inline-flex items-center gap-1 sm:gap-1.5 text-white text-[8px] sm:text-[10px] font-bold px-1.5 sm:px-2.5 py-1 sm:py-1.5 rounded-full bg-gradient-to-r from-amber-400 to-orange-500 shadow-lg shadow-orange-500/30 backdrop-blur-sm">
          ...
          >
            <div className="relative w-full h-full">
              ...
              {!imageLoaded && !imageError && (
                <div className="absolute inset-0 bg-gradient-to-r from-gray-100 via-gray-200 to-gray-100 animate-shimmer rounded-xl sm:rounded-2xl" />
              )}
        ...
        <div className="absolute bottom-2 sm:bottom-3 left-1/2 -translate-x-1/2 z-30 flex items-center gap-2 sm:gap-3">
          {/* Wishlist Button */}
          <button
            onClick={(e) => {
              e.preventDefault();
            ...
            className={`
              p-2 sm:p-2.5 rounded-full transition-all duration-500 
          ...
          >
            {isAddingToWishlist ? (
              <div className="h-3 w-3 sm:h-4 sm:w-4 rounded-full border-2 border-rose-500 border-t-transparent animate-spin" />
            ) : (
              <Heart
                size={14}
                className={`
          ...
          <Link
            ...
            title="Quick View"
          >
            <Eye size={14} className="text-gray-700 group-hover/btn:text-orange-500 transition-colors sm:w-4 sm:h-4" />
          </Link>
        </div>

        {/* Out of Stock Overlay */}
        {!isInStock && (
          <div className="absolute inset-0 z-40 flex items-center justify-center bg-white/70 backdrop-blur-[1px] rounded-2xl sm:rounded-3xl">
            <span className="inline-flex items-center gap-1 sm:gap-1.5 text-[10px] sm:text-xs font-semibold px-2 sm:px-3 py-1 sm:py-1.5 bg-white/90 backdrop-blur-md rounded-full shadow-lg border border-gray-100 text-gray-700">
      ...
      {/* Content Section */}
      <div className="flex-1 flex flex-col px-3 sm:px-4 pb-3 sm:pb-4 pt-1.5 sm:pt-2 bg-white">
        
        {/* Product Name */}
        <Link 
          href={`/products/${product.id}`} 
          onClick={() => onViewTrack && onViewTrack(product.id)}
          className="group/link mb-1.5 sm:mb-2"
        >
          <h3 className="font-medium text-gray-900 text-xs sm:text-sm leading-snug line-clamp-2 h-8 sm:h-10 hover:text-orange-600 transition-all duration-300">
        ...
        <div className="flex items-center justify-between mb-2 sm:mb-2.5">
          ...
          {discountPercentage > 0 && (
            <div className="flex items-center gap-1 bg-emerald-50 px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-full">
        ...
        {showActions && (
          <button
            ...
            className={`
              relative w-full py-2 sm:py-3 px-2 sm:px-4 rounded-lg sm:rounded-xl font-semibold text-xs sm:text-sm
              transition-all duration-500 ease-out
              flex items-center justify-center gap-1 sm:gap-2
              overflow-hidden group/btn
              ${isInStock 
                ? 'bg-emerald-500 hover:bg-orange-500 text-white shadow-md sm:shadow-lg shadow-emerald-500/30 hover:shadow-orange-500/30' 
          ...
          >
            {isAddingToCart ? (
              <>
                <div className="h-3 w-3 sm:h-4 sm:w-4 rounded-full border-2 border-white/30 border-t-white animate-spin" />
                <span className="text-xs sm:text-sm">Adding...</span>
              </>
            ) : (
              <>
                <ShoppingCart size={12} className="sm:w-4 sm:h-4 relative z-10 transition-transform group-hover/btn:scale-110" />
                ...
                {isInStock && (
                  <Plus 
                    size={10} 
                    className="sm:w-3.5 sm:h-3.5 opacity-0 group-hover/btn:opacity-100 transition-all duration-300 group-hover/btn:translate-x-0 -translate-x-2" 
                  />
                )}
              </>
            )}
